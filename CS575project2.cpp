...
main( int argc, char *argv[ ] )
{
	...
	for( int t = 0; t < NUMSTEPS; t++ )
	{
		...
		for( int i = 0; i < NUMBODIES; i++ )
		{
			...
			//#pragma omp parallel for reduction(+:fx, fy, fz), shared(Bodies, bi), schedule(static)
			//#pragma omp parallel for reduction(+:fx, fy, fz), shared(Bodies, bi), schedule(dynamic)
	...
	printf("%10.2lf\n", (float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(1000000 * (time1 -time0)));
